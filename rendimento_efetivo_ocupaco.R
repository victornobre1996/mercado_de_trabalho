...
for (i in seq_along(mylist)) {
  ...
  a1 <- svyby(formula =~(VD4020_real = VD4020*Efetivo), by = ~interaction(VD4009 == "Empregador" & VD4012=="Contribuinte"), 
              design = p, FUN = svymean, na.rm = TRUE, na.rm.all = T)
 ...
 # a4 <- svyby(formula =~(VD4020_real = VD4020*Efetivo), by = ~interaction(VD4009 == "Conta-própria" &
  ...
  assign(paste0("pnad_rendiento_ocupados_ok",i), a1)            
...
interaction(pnad_2019$variables$VD4009 == "Empregador" &
              pnad_2019$variables$VVD4012 =="Contribuinte", drop = T)
